...
int main() {
    ...
    while (1) {
        ...
        value = recv(sock, buffer, BUFFER_SIZE, MSG_DONTWAIT);
        if (value > 0) {
...
// message si le bouton est appuyé
#define MESSAGE_ON "1\n"
// message si le bouton est à son état normal (pas appuyé)
#define MESSAGE_OFF "0\n"
// taille du buffer
#define BUFFER_SIZE 1024
// adresse i2c
...
#define ADR_SYS_MATRICE 0x20
// adresse d'affichage de la matrice
#define ADR_AFFICHAGE_MATRICE 0x80
// Pin gpio du bouton (signal)
#define BTN_GPIO 17
...
int main() {
    ...
    while (1) {
        ...
        // Attendre la réponse du conteneur
        value = recv(sock, buffer + bytes_read, BUFFER_SIZE - bytes_read - 1, MSG_DONTWAIT);
        if (value > 0) {
            bytes_read += value;
            buffer[bytes_read] = '\0'; // Assurez-vous que le buffer est correctement terminé
 
            // Traiter chaque ligne de message dans le buffer
            char *line = strtok(buffer, "\n");
            while (line != NULL) {
                printf("Message reçu : %s\n", line); // Débogage : Afficher chaque ligne de message
